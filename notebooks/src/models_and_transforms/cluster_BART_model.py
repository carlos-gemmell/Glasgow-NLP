...
class DecoderLayer(nn.Module):
    def __init__(self, config: BartConfig):
        ...
        self.self_attn_layer_norm = LayerNorm(self.embed_dim)
        self.encoder_attn = Hierarchical_Attention(
            self.embed_dim,
            dropout=config.attention_dropout,
        )
    ...
    def forward(
        ...
        x, queries, keys, slot_vectors, slot_assignment_scores, slot_attention_scores = self.encoder_attn(
            x, 
            encoder_hidden_states, 
            encoder_hidden_states, 
...
#         print('in transformer src2', src2)
    ...
    def forward(self, queries, keys, values, cutof='soft', key_padding_mask=None):
        '''
...
#         print("scaled_values", scaled_values)
        ...
        helper_outputs = {
            ...
            'keys': K.permute(1,0,2),
            'slot_vectors': slot_vectors.permute(1,0,2),
            'slot_assignment_scores': slot_assignment_scores,
            'slot_attention_scores': slot_attention_scores
        }
